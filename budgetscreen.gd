...
func _ready():
	...
	# sliders
	$C/M/R/CHiring/HiringLabel.text = "Hiring intensity: " + str(GameLogic.IntensityPercent(GameLogic.IntensityHiring)) + "%"
	$C/M/R/CHiring/HiringSlider.max_value = 100
	$C/M/R/CHiring/HiringSlider.value = GameLogic.IntensityHiring
	$C/M/R/CUpskill/UpskillLabel.text = "Upskilling intensity: " + str(GameLogic.IntensityPercent(GameLogic.IntensityUpskill)) + "%"
	$C/M/R/CUpskill/UpskillSlider.max_value = 100
	$C/M/R/CUpskill/UpskillSlider.value = GameLogic.IntensityUpskill
	$C/M/R/CTech/TechLabel.text = "R&D intensity: " + str(GameLogic.IntensityPercent(GameLogic.IntensityTech)) + "%"
	...
	if diff > 0: $C/M/R/SkillChange.text = "+" + str(diff) + "% in the last six months"
	else: $C/M/R/SkillChange.text = str(diff) + "% in the last six months"
	$C/M/R/CTech2/TechBar.value = GameLogic.Technology
	diff = int(GameLogic.Technology - GameLogic.TechnologyMonthsAgo[0])
	if diff > 0: $C/M/R/TechChange.text = "+" + str(diff) + "% in the last six months"
	...
	if diff > 0: $C/M/R/ExperienceChange.text = "+" + str(diff) + "% in the last six months"
	else: $C/M/R/ExperienceChange.text = str(diff) + "% in the last six months"
